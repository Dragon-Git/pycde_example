...
1.module的input和output 对应pycde中的class成员；
...
1.generator表示接下来内容是module核心要做的事情;
2.访问instance members不是通过self,而是ports;
...
1.System 使用class 创建hardware;
2.其他在generator中创建的module会自动递归的生成；
3.name 默认是top module name
  output_directory默认是name
  compile方法将生成一个build_package，此package包含生成的系统，已知的有RTL,filelist,tcl
...
1.self.a, self.b是PyCDE Signal
2.被实例化模块的输出，可以使用or_inst.c的方式引用
...
1.CIRCT目标是生成hardware
2.CIRCT有自己的type，通过pycde.types.Type引用
3.PyCDE signals代表目标device的值
4.Signals have a particular `Type` (which is distinct from the signal objects' Python `type`) stored in their
`type` instance member. `Type`s are heretofor referred to interchangeably as "PyCDE type", "CIRCT type", or "Type".
没看明白啥意思？
...
1.所有信号都继承自pycde.signals.Signal，并由Type进一步特化；
展开解释：pycde中操作的不是值，而是硬件信号；之所以可以直接写|。+，^，是因为signal的子类重载了python运算符，每个运算都会生存对应的CIRCT op;
...
1.CIRCT的type不只是数据类型，还可能包含signaling机制；
2.一个Signal看起来像wire，语义上可能是clock，reset，handshake；
...
1.python没有为宽，硬件是有位宽的，所以pycde会用刚好表示这个数的位宽；
2.但有些场景下，这种自动推断的位宽是错的，此时需要开发者手动指定位宽；

### Scalars

`Bits(width)` models a bitvector. Allows indexing, slicing, bitwise operations, etc. No math operations.
1.Bits(width)表示bitvector，支持位选，切片，按位逻辑运算，不支持算术场景

`UInt(width)`, `SInt(width)` math.
1.UInt(width), SInt(width)表示数值，用在算术场景
...
1.代表硬件中的数组
2.Bits(32) * 10 代表1维数据，长度是10,每个元素是32bit；
3.Bits(32) * 10 * 12,代表2维数组，10*12,每个元素是32bit；
...
```
pycde中的struct类似C 的struct，是一个复合硬件类型；
struct可以有方法，例如示例中的get_b_xor
struct也能作为端口，例如示例中的inp1=Input(ExStruct)
...
arrays.
1.pycde支持numpy子集，可以用来作复杂的多维数组的reshaping和transformation；
...
functions.
1.关键对象：NDArray
2.所有变换先发生在python层，最后一次性to_circt(),中间过程不可见，调试不友好，但性能高；
...
1.没有@generator的module就是external module
2.external module是pycde和已经存在的verilog module之间交互的方式；
3.pycde只会声明和实例化，实现由verilog完成；
4.特别注意pycde不会检查端口是否匹配；
...
1.使用modparams告知pycde 此module是一个参数化的module
2.modparams做了2件事：缓存参数化的函数  以及  自动生成包含parameter的module name
3.pycde的参数化是python层面的参数化，所有参数化在python阶段就已经展开完，verilog看到的是“定型后的模块”
...
```
1.对接带有parameter的外部模块，如果有@modparams，但是没有@generator，pycde会把它当作有parameter的外部SV module，自动生成SV的实例化语句
